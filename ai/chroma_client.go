...
func (c *ChromaClient) EnsureCollection(ctx context.Context, name string) (string, error) {
    ...
    if resp.StatusCode == 200 {
        var out struct{ Collections []chromaCollection `json:"collections"` }
        if err := json.NewDecoder(resp.Body).Decode(&out); err == nil {
            if len(out.Collections) > 0 { return out.Collections[0].ID, nil }
        }
    }
    // Create
...
func (c *ChromaClient) Upsert(ctx context.Context, collectionID string, items []UpsertItem) error {
    ...
    req, _ := http.NewRequestWithContext(ctx, http.MethodPost, c.BaseURL+"/api/v1/collections/"+collectionID+"/upsert", bytes.NewReader(body))
    req.Header.Set("Content-Type", "application/json")
    resp, err := c.http.Do(req)
    if err != nil { return err }
    defer resp.Body.Close()
...
func (c *ChromaClient) Query(ctx context.Context, collectionID string, queryEmbeddings [][]float32, topK int) (QueryResult, error) {
    ...
    defer resp.Body.Close()
    if resp.StatusCode != 200 { return QueryResult{}, fmt.Errorf("chroma query failed: %s", resp.Status) }
    var out QueryResult
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return QueryResult{}, err }
    return out, nil
}
